...
# load top tables
sensitives <- read.csv("C:/Users/paucl/Documents/TFM/DEA/results/drug_sensitive/toptable_olaparib_vs_control_allsensitive.csv")
resistants <- read.csv("C:/Users/paucl/Documents/TFM/DEA/results/drug_resistant/toptable_olaparib_vs_control_allresistant.csv") 
...
Això era per comparar els Wald stat entre dins i fora del gene set per resistents i sensibles per separat
```{r}
# # take wald stat for all genes inside and outside the gene sets
# zin <- lapply(hallmark ,
#             \(x) subset(all ,  all$symbol %in% x)[grep("stat",
#                                                           colnames(all))])
# zout <- lapply(hallmark ,
#             \(x) subset(all ,  !(all$symbol %in% x))[grep("stat",
#                                                           colnames(all))])
# 
# 
...
# wilcox.s <- c()
# for(geneset in 1:50){
#      ins <- unlist(zin[[geneset]][1])
#      outs <- unlist(zout[[geneset]][1])
#      length(ins) <- length(outs)
#      total <- cbind(ins, outs)
#      wilcox.s <- c(wilcox.s, wilcox.test(ins,outs, data=total)$p.value)
# }
# 
# wilcox.r <- c()
# for(geneset in 1:50){
#      ins <- unlist(zin[[geneset]][2])
#      outs <- unlist(zout[[geneset]][2])
#      length(ins) <- length(outs)
#      total <- cbind(ins, outs)
#      wilcox.r <- c(wilcox.r, wilcox.test(ins,outs, data=total)$p.value)
#  }
# 
# meanlogFC$wilcox.s <- wilcox.s
# meanlogFC$wilcox.r <- wilcox.r
# 
# meanlogFC$wilcox.signif <- ifelse(meanlogFC$wilcox.s < 0.05, ifelse(meanlogFC$wilcox.r<0.05, "Sensitive & Resistant", "Sensitive"), ifelse(wilcox.r<0.05, "Resistant", "None"))
...
#   #                              "ns" = "Grey"))+
#   scale_color_manual(values = c("Sensitive & Resistant" = "#8500AC",
#                               "Sensitive" = "#1984c5",
#                               "Resistant" = "#c23728",
#                               "None" = "Grey"))+
#   scale_shape_manual(values= c("< 0.05" = 17,
#                                "ns" = 16))+
#   theme_classic()+
...
#                                title = "GSEA significant"),
#          shape = guide_legend(override.aes = list(size=5),
#                               title= "Differential \n treatment effect \n by response"))
# ggsave("C:/Users/paucl/Documents/TFM/DEA/results/bidimensional/bidimensionalGSEA.pdf", units="in", limitsize = FALSE, width=10, height=6)


```
```{r}
# Versió de la violeta
...
ggplot(meanlogFC, 
  ...
  geom_text_repel(box.padding = 0.5, 
                  max.overlaps = Inf,
                  size=3)+
  scale_color_manual(values = c("Sensitive & Resistant" = "#4b9600",
                              "Sensitive" = "#1984c5",
                              "Resistant" = "#C309C7",
                              "None" = "darkgrey"),
  ...
  guides(colour = guide_legend(override.aes = list(size=5),
         ...
         shape = guide_legend(override.aes = list(size=5),
                              title= "Differential \n treatment effect \n by response"))+
  coord_fixed(ratio=1)
ggsave("C:/Users/paucl/Documents/TFM/DEA/results/bidimensional/bidimensionalGSEAnew.png", units="in", limitsize = FALSE, width=10, height=6)
```
