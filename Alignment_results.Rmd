...
mix <- inner_join(cnag.h, mycounts, by="symbols", suffix=c(".CNAG", ".my"))
mix <- mix[order(colnames(mix))]

# Correlation without filtering
corr_raw <- cor(mix[grep(".CNAG", colnames(mix))], mix[grep(".my", colnames(mix))])
...
corr_filtered <- cor(mix.z[grep(".CNAG", colnames(mix.z))], mix.z[grep(".my", colnames(mix.z))])
