...
- És possible que els gens implicats en la resistència variin segons els PDX i que aleshores s'elimini al considerar el PDX com a adjustment variable.

```{r setup, warning=FALSE, message=FALSE, include=FALSE}
knitr::opts_chunk$set(cache = T, warning=F, message=F)

```
fer el grafic d'alpha per l'altre contrast i fer bonics els gràfics del pre-ranked GSEA pels dos contrastos
...
# Create a dataframe with POL2RJ TMM and phenodata information
plotthis <- cbind.data.frame("POL2RJ"=t(TMM[grep("POL2RJ", rownames(TMM)),]), 
                             phenodata[,c("Group", "Drug", "Response", "PDX_name", "Original_PDX")])

# Look for differences in POL2RJ expression
sensitiveALL <- subset(plotthis, Response=="Sensitive")$POL2RJ
resistantALL <- subset(plotthis, Response=="Resistant")$POL2RJ
...
ggplot(plotthis, aes(x=Response, y=POL2RJ, fill=Response))+
  geom_boxplot()+
  geom_point(aes(group= PDX_name))+
  theme_classic()
t.test(plotthis[plotthis$Response=="Sensitive", "POL2RJ"], 
       plotthis[plotthis$Response=="Resistant", "POL2RJ"], 
       paired = T,
       alternative = "greater",
       var.equal = T)

# # Ratio POL2RJ expression in olaparib/control
# ratio <- tapply(plotthis$POL2RJ, plotthis$PDX_name, \(x){x[1]/x[2]})
# 
...
# 
# # Treatment effect on POL2RJ expression is higher in sensitive
...
# geom_point(size = 2)
# 
# ggplot(plotthis, aes(x = Drug, y = POL2RJ)) + 
