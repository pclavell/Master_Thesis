...
packages <- c("dplyr","EDASeq", "stringr", "ggbio", "ggplot2","limma", "EnhancedVolcano", "biomaRt", "edgeR", "tweeDEseq", "clusterProfiler","DESeq2", "pheatmap", "fgsea", "noisyr")
...
```{r}
## Use noisyr package to remove noise from the count table
...
counts2 <- remove_noise_from_matrix(counts,
                                    thresholds)
counts <- counts2
...
factoextra::fviz_pca_ind(pcaResults, geom.ind=c("point"), repel=T, habillage= phenodata$Response, max.overlaps=5)
...
png("C:/Users/paucl/Documents/TFM/DEA/results/exploratoryHEATMAP.png", width=12, height=5, units="in", res=1024)
...
input <- t(scale(t(na.omit(as.data.frame(TMM)))))
...
dds0 <- DESeqDataSetFromMatrix(countData = as.matrix(counts2), 
                               colData = phenodata, 
                               design = design)
